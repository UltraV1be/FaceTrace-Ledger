...
export function downloadCsvReport(result: PipelineResult): void {
  ...
  const row = [
    ...
    escapeCsv(result.record?.result_type || result.best_match?.result_type || "GENERAL_WEB_RESULT"),
    escapeCsv(result.record?.is_social_media ?? result.best_match?.is_social_media ?? false),
    escapeCsv(result.record?.social_platform || result.best_match?.social_platform || "none"),
    ...
    escapeCsv(result.record?.similarity_threshold ?? 0.65),
