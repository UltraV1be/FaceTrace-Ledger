...
export const PipelineFailureModal: React.FC<PipelineFailureModalProps> = ({
  ...
  const stageNumber = failedStageObj?.number || errorDetails?.stage_name?.slice(0, 2) || '05';
  const stageName = failedStageObj?.name || errorDetails?.stage_name || 'COMPARE';

  // Extract human-readable valid reason
  const validReason = errorDetails?.valid_reason || 
                      errorDetails?.error_message || 
                      errorMessage || 
                      failedStageObj?.message || 
                      'An unexpected termination occurred during forensic pipeline execution.';
  ...
  const getRecommendation = () => {
    switch (errorCode) {
      ...
      default:
        return 'Review system diagnostics below and attempt trace restart or test with a new benchmark portrait.';
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-2xl bg-goa-cream text-goa-dark border-4 border-goa-dark shadow-[10px_10px_0px_0px_#ff0080] p-6 sm:p-8 max-h-[92vh] overflow-y-auto space-y-6">
        ...
        {/* Valid Reason Card */}
        <div className="p-4 sm:p-5 bg-red-100/90 border-2 border-red-600 shadow-brutal space-y-3 font-mono">
          <div className="flex items-center justify-between border-b border-red-300 pb-2">
            <span className="text-[11px] font-black uppercase tracking-wider text-red-900 flex items-center gap-1.5">
              ...
              TERMINATION CAUSE & VALID REASON
            </span>
          ...
          {/* Stage failure summary metric if available */}
          {errorDetails?.threshold !== undefined && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2 border-t border-red-200 text-[11px] text-red-900 font-semibold">
              <div>THRESHOLD: <span className="font-bold text-red-700">{errorDetails.threshold}</span></div>
              <div>CANDIDATES INSPECTED: <span className="font-bold text-red-700">{errorDetails.candidates_checked || candidatesList.length}</span></div>
              <div>STAGE: <span className="font-bold text-red-700">{stageName}</span></div>
            </div>
          )}
        </div>

        {/* Execution Stage Status Mini-Bar */}
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-7 gap-1.5 font-mono text-[10px] text-center">
            {stages.map((st) => {
              ...
              const isWait = st.status === 'waiting' || st.status === 'idle';
        ...
        <div className="pt-2 border-t-2 border-goa-dark flex flex-wrap items-center justify-end gap-3 font-mono text-xs font-bold uppercase">
          <button
            onClick={() => {
              onClose();
              onRestart();
            }}
            className="px-4 py-3 bg-goa-pink text-goa-cream hover:bg-[#FF006E] border-2 border-goa-dark shadow-brutal flex items-center space-x-1.5 transition cursor-pointer active:translate-x-0.5 active:translate-y-0.5"
          >
          ...
          <button
            onClick={onClose}
            className="px-4 py-3 bg-white text-goa-dark hover:bg-goa-cream border-2 border-goa-dark shadow-brutal transition cursor-pointer"
          >
            [ ✕ DISMISS POPUP ]
          </button>
        </div>
